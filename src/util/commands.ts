...
export function parseCommand(message: Message): { command: Command; args: Args } {
    ...
    const command = commands.find(
        c => c.options.aliases.includes(cmd.value.toLowerCase()) ||
            c.options.aliases
    ...
    if (command.options.mod && !message.member?.permissions.has(Permissions.FLAGS.MANAGE_GUILD) && !isOwner(message.author)) return { command: null, args: null };
...
export async function handleMessageCommand(message: Message, command: Command, args: Args): Promise<void> {
    ...
    if (
        (command.options.args?.length || (command.args && typeof command.args === 'function')) &&
        typeof params.match === 'undefined'
    ) {
...
async function runCooldowns(data: CommandInteraction | Message, command: Command): Promise<boolean> {
    ...
    if (cooldownUsers.has(user.id)) {
        ...
        if (now < expiration) {
            ...
            const text = `Please wait ${pluralize('second', Number(seconds))} before using \`${process.env.BOT_PREFIX + command.options.name}\` again.`;
