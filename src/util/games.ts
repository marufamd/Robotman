...
export class HangmanGame {
	...
	public get board(): string {
		...
		const str = stripIndents`
            ...
            ${p(4)} |       ${h(3, Hangman.EMOJIS.LEFT_HAND)}${h(2, `${this.incorrect === 2 ? ' ' : ''}${Hangman.EMOJIS.SHIRT}`)}${h(
			4,
			Hangman.EMOJIS.RIGHT_HAND
		)} 
...
export class TicTacToeGame {
	...
	public createBoard() {
		...
		for (let i = 0; i < 3; i++) {
			...
			for (let j = 0; j < 3; j++) {
				buttons.push(new MessageButton().setCustomId(id.toString()).setLabel('\u200b').setStyle('SECONDARY'));
