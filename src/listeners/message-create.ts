...
export default class implements Listener {
    ...
    public async handle(message: Message) {
        const hasPermission = (permission: PermissionResolvable) => (message.channel as TextChannel).permissionsFor(message.client.user).has(permission);

        if (message.channel.type === 'GUILD_NEWS' && hasPermission(Permissions.FLAGS.MANAGE_MESSAGES) && Object.values(Channels.NEWS).includes(message.channel.id)) {
