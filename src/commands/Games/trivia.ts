...
const { CATEGORIES, DEFAULT_AMOUNT, MAX_UNANSWERED, MAX_TIME } = Trivia;
...
export default class implements Command {
	public options: CommandOptions = {
		...
		example: ['dccomics 20', 'marvelcomics'],
		args: [
			{
				name: 'category',
				type: keys,
	...
	private async run(data: Message | CommandInteraction, category: Category, amount: number) {
		...
		if (amount > questions.length) {
			await data.channel.send(
				`There are not enough questions written for this length of a game. Amount will be defaulted to ${DEFAULT_AMOUNT}.`
			);
		...
		for (let i = 0; i < amount; i++) {
			...
			const row = new MessageActionRow().addComponents(new MessageButton().setCustomId('stop').setStyle('DANGER').setLabel('Stop'));
