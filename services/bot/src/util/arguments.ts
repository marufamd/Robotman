...
export const ArgumentTypes: { [key: string]: ArgumentTypeCaster } = {
	...
	codeBlock: (_, arg) => {
		if (!arg) return null;
		return arg.replace(/^\s*```(js|javascript|ts|typescript|sql)?/, '').replace(/```$/, '');
	},
		...
		if (isNaN(parsed)) {
			if (['next', 'last'].includes(arg.toLowerCase())) {
				...
				const month = arg.match(
					/(jan(uary)?|feb(ruary)?|mar(ch)?|apr(il)?|may|june?|july?|aug(ust)?|sep(tember)?|oct(ober)?|nov(ember)?|dec(ember)?)/gi
				);
	...
	timezone: (_, arg) => {
		...
		const target = closest(
			arg.toLowerCase().split(' ').join('_'),
			Timezones.map((t) => t.toLowerCase())
		);
	...
	color: (_, arg) => {
		...
		if (split.length === 3 && !isNaN(parseRGB(split[0]))) {
			...
			resolve = resolve.length !== 6 && !isNaN(parseInt(resolve)) ? parseInt(resolve) : resolve;
		}
...
async function prompt(message: Message, arg: ArgumentOption, retries: number): Promise<string> {
	const text =
		retries >= 1 ? PromptOptions.TEXT.RETRY(await fnOrAny(arg.prompt, message)) : PromptOptions.TEXT.START(await fnOrAny(arg.prompt, message));
