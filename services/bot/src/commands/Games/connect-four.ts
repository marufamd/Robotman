...
export default class implements Command {
	...
	private async run(data: Message | CommandInteraction) {
		...
		const game = new ConnectFourGame().addPlayers(getUser(data), playerData.player).makeBoard();
		...
		const embed = new Embed().setColor(Colors.CONNECT_FOUR).setDescription(game.currentBoard);
	...
	private generateButtons(disabled = false) {
		...
		for (let i = 0; i < 2; i++) {
			...
			for (let j = 0; j < 4; j++) {
				...
				if (count === 7) {
					button = new MessageButton().setCustomId('stop').setLabel('Forfeit').setStyle('DANGER').setDisabled(disabled);
				} else {
					const id = (++count).toString();
					button = new MessageButton().setCustomId(id).setLabel(id).setStyle('PRIMARY').setDisabled(disabled);
				}
